...
reimannShaderDetailsObject = function(name) {
    ...
    this.drawLabelOnCanvas = function(rn, rd,i, hint) {
        ...
        var arr = xform.inverse().doit3Part(z);
        z = arr[2]
        var num = arr[0];
        var den = arr[1];
        var hint = hint 
        * that.safeMultiply(xform.a.x)
        * that.safeMultiply(xform.b.x)
        * that.safeMultiply(xform.c.x)
        * that.safeMultiply(xform.d.x)
        ;        
        ...
        if (i == 0) {
            if (num.x * den.x !=0) {
                nn = Math.abs(num.x) * hint;
                dd = Math.abs(den.x) * hint;
                var arr2;
                arr2 =that.reduce(nn,dd);
                // var arr2;
                // if (Math.abs(num.x) < 1.0 || Math.abs(den.x < 1.0)) {
                //     arr2 = Math.abs(num.x) < Math.abs(den.x) ?
                //     that.reduce(hint*Math.abs(num.x/num.x), hint*Math.abs(den.x/num.x)) :
                //     that.reduce(hint*Math.abs(num.x/den.x), hint*Math.abs(den.x/den.x))
                //     ;
                // }
                // else
                //     arr2 =that.reduce(hint*Math.abs(num.x), hint*Math.abs(den.x));
                console.log(num.x, den.x, arr2[0], arr2[1]);
                num.x = arr2[0]*Math.sign(num.x * den.x);
                den.x = arr2[1];
            }
            txt = num.displayString() + "/" + den.displayString();
    ...
    this.updateFareyLabelsForMobiusTransform = function() {
        ...
        that.drawLabelOnCanvas(1.0,2,0.0, 1.0);
        that.drawLabelOnCanvas(1.0,3,0.0, 3.0);
        that.drawLabelOnCanvas(1.0,4,0.0, 1.0);
        that.drawLabelOnCanvas(1.0,5,0.0, 5.0);
        ...
        that.drawLabelOnCanvas(1.0,1,0.0, 1.0);
        that.drawLabelOnCanvas(2.0,1,0.0, 1.0);
        that.drawLabelOnCanvas(3.0,1,0.0, 3.0);
        that.drawLabelOnCanvas(4.0,1,0.0, 1.0);
        that.drawLabelOnCanvas(5.0,1,0.0, 5.0);
        that.drawLabelOnCanvas(-1.0,1,0.0, 1.0);
        that.drawLabelOnCanvas(-2.0,1,0.0, 1.0);
        that.drawLabelOnCanvas(-3.0,1,0.0, 3.0);
        that.drawLabelOnCanvas(-4.0,1,0.0, 1.0);
        that.drawLabelOnCanvas(-5.0,1,0.0, 5.0);
        that.drawLabelOnCanvas(0.0,1, 1.0, 1.0);
        that.drawLabelOnCanvas(0.0,1, 2.0, 1.0);
        that.drawLabelOnCanvas(0.0,1, 3.0, 3.0);
        that.drawLabelOnCanvas(0.0,1, 4.0, 1.0);
        that.drawLabelOnCanvas(0.0,1, 5.0, 5.0);
